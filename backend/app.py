...
class VirtualMemorySimulator:
   ...
   def create_process(self, pid, pages_needed):
       ...
       # allocate initial pages to physical memory
       ...
       # create entries for remaining pages (not yet in memory)
   ...
   def handle_page_fault(self, pid, page_number, future_accesses=None):
       ...
       # load page into available frame
       if self.free_frames:
           frame = self.free_frames.pop(0)
   ...
   def select_clock_victim(self):
       checked = 0
       total_pages_in_mem = sum(len([p for p in proc['page_table'].values() if p['valid']]) 
                              for proc in self.processes.values())
       
       if total_pages_in_mem == 0:
           return None
           
       # circular search for victim
       while True:
           ...
           if frame_info:
               ...
               if key in self.clock_bits:
                   if self.clock_bits[key] == 0:
                       ...
                       return {'frame': frame_info['frame'], 'pid': pid, 'page': page_num, 'reason': 'clock hand found page with reference bit 0'}
                   else:
                       # give second chance
   ...
   def select_optimal_victim(self, future_accesses):
       # optimal algorithm needs future knowledge
       ...
       # victim is page used furthest in future
       if victim:
   ...
   def update_access_info(self, pid, page_number):
       ...
       # update algorithm-specific access info
   ...
   def update_tlb(self, pid, page_number, frame):
       ...
       # simple fifo replacement for tlb
   ...
   def update_working_set(self, pid, page_number):
       ...
       current_time = time.time()
       window_size = 10.0  # working set window in seconds
   ...
   def detect_thrashing(self):
       ...
       # simple thrashing detection: high fault rate
...
def run_demo():
   try:
       ...
       # create demo processes if none exist
...
def random_access():
   try:
       ...
       for _ in range(count):
           ...
           addr = random.randint(0, 0x7FFF)  # random address in reasonable range
