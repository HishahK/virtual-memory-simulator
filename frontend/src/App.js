...
function App() {
 ...
 const [showAccessPattern, setShowAccessPattern] = useState(false);
 const [pageFaultAnimation, setPageFaultAnimation] = useState(null);
 ...
 const [showProcessLayouts, setShowProcessLayouts] = useState(false);
 ...
 const calculateSystemResources = useCallback(() => {
   ...
   if (stats.memory_accesses > 0) {
     setPerformanceChart(prev => ({
       pageFaults: [...prev.pageFaults.slice(-19), stats.page_faults],
       hitRatios: [...prev.hitRatios.slice(-19), stats.hit_ratio * 100],
       timestamps: [...prev.timestamps.slice(-19), new Date().toLocaleTimeString()]
     }));
   }
 }, [memoryState, tlbState]);

 const updateAccessHistory = useCallback((access) => {
   setAccessHistory(prev => [...prev.slice(-50), {
     ...access,
     timestamp: new Date(),
     id: Date.now()
   }]);
 ...
 const triggerPageFaultAnimation = useCallback((frameIndex) => {
   setPageFaultAnimation({ frameIndex, timestamp: Date.now() });
   setTimeout(() => setPageFaultAnimation(null), 2000);
 }, []);

 const updateProcessLayouts = useCallback(() => {
   if (!memoryState?.processes) return;

   const layouts = {};
   Object.entries(memoryState.processes).forEach(([pid, process]) => {
     const totalPages = process.pages_needed;
     const allocatedPages = process.allocated_pages;
     const virtualPages = [];
     
     for (let i = 0; i < totalPages; i++) {
       const pageEntry = process.page_table?.[i.toString()];
       virtualPages.push({
         pageNumber: i,
         allocated: i < allocatedPages,
         valid: pageEntry?.valid || false,
         frame: pageEntry?.frame || null,
         dirty: pageEntry?.dirty || false,
         referenced: pageEntry?.referenced || false
       });
     }
     ...
     layouts[pid] = {
       ...
       allocatedPages,
       virtualPages,
       workingSetSize: workingSets?.[pid]?.size || 0
     };
   });
   
   setProcessLayouts(layouts);
 }, [memoryState, workingSets]);
 ...
 useEffect(() => {
   ...
   updateProcessLayouts();
 }, [calculateSystemResources, updateProcessLayouts]);
   ...
   try {
     ...
     if (response.data.success) {
       ...
       updateAccessHistory({
         ...
         pid: response.data.memory_state.current_pid || 'new',
         pages: parseInt(newProcessPages)
       });
     } else {
 ...
 const translateAddress = async () => {
   ...
   try {
     ...
     if (response.data.result?.page_fault) {
       triggerPageFaultAnimation(response.data.result.frame);
     }
 ...
 const resetSimulator = async () => {
   ...
   try {
     ...
     if (response.data.success) {
       ...
       setShowAccessPattern(false);
       setShowPerformanceChart(false);
       setShowProcessLayouts(false);
       setBenchmarkResults(null);
       setLastAccessTime(null);
       setStartTime(new Date());
       setAccessHistory([]);
       setPageFaultAnimation(null);
       setPerformanceChart({ pageFaults: [], hitRatios: [], timestamps: [] });
       setProcessLayouts({});
       setError(null);
       if (demoIntervalRef.current) {
         clearInterval(demoIntervalRef.current);
 ...
 const runDemo = async () => {
   ...
   try {
     ...
     if (response.data.success) {
       ...
       updateAccessHistory({
         type: 'demo_run',
         results: response.data.demo_results?.length || 0
       });
       ...
       {(memoryState.physical_memory || []).map((frame, index) => (
         <div 
           ...
           className={`memory-frame ${frame ? 'occupied' : 'free'} ${
             pageFaultAnimation?.frameIndex === index ? 'page-fault-animation' : ''
           }`}
           style={{
 ...
 const renderStats = () => {
   ...
   return (
     <div className="stats-panel">
       ...
       {workingSets && (
         <div className="working-set-visualization">
           <h4>Working Set Visualization</h4>
           {Object.entries(workingSets).map(([pid, ws]) => (
             <div key={pid} className="working-set-chart">
               <h5>Process {pid}</h5>
               <div className="working-set-bar">
                 <div 
                   className="working-set-fill"
                   style={{ width: `${(ws.size / 8) * 100}%` }}
                 >
                   {ws.size} pages
                 </div>
               </div>
               <div className="working-set-pages">
                 {(ws.current_set || []).map(pageNum => (
                   <span key={pageNum} className="working-set-page">P{pageNum}</span>
                 ))}
               </div>
             </div>
           ))}
       ...
       {tlbState?.tlb && (
         <div className="tlb-visualization">
           <h4>TLB Hit/Miss Indicator</h4>
           <div className="tlb-status">
             <div className={`tlb-indicator ${translationResult?.tlb_hit ? 'hit' : 'miss'}`}>
               {translationResult?.tlb_hit ? 'TLB HIT' : 'TLB MISS'}
             </div>
           </div>
           <div className="tlb-entries-visual">
             {Array.from({ length: 4 }, (_, i) => {
               const entries = Object.entries(tlbState.tlb);
               const entry = entries[i];
               return (
                 <div key={i} className={`tlb-slot ${entry ? 'occupied' : 'empty'}`}>
                   <div className="slot-number">Slot {i}</div>
                   {entry && (
   ...
   <div className="App">
       ...
       <div className="left-panel">
         ...
         <div className="control-section">
           ...
           <div className="analysis-buttons">
             ...
             <button 
               onClick={() => setShowAccessPattern(true)} 
               disabled={connectionStatus !== 'connected'}
               className="analysis-btn"
             >
               <Activity size={16} /> Access Pattern
             </button>
             <button 
               ...
               className="analysis-btn"
             >
               <TrendingUp size={16} /> Performance Chart
             </button>
             <button 
               onClick={() => setShowProcessLayouts(true)} 
               disabled={connectionStatus !== 'connected'}
               className="analysis-btn"
             >
               <Database size={16} /> Process Layouts
             </button>
             <button 
     ...
     {showAccessPattern && (
       <div className="comparison-modal">
         <div className="modal-content">
           <div className="modal-header">
             <h3><Activity size={20} /> Memory Access Pattern</h3>
             <button onClick={() => setShowAccessPattern(false)} className="close-btn">×</button>
           </div>
           <div className="comparison-table">
             <div className="access-timeline">
               {accessHistory.slice(-20).map((access, index) => (
                 <div key={access.id} className={`access-event access-${access.type}`}>
                   <div className="access-time">{access.timestamp.toLocaleTimeString()}</div>
                   <div className="access-details">
                     {access.type === 'address_translation' && (
                       <span>PID {access.pid}: 0x{access.virtualAddress.toString(16)} 
                         {access.result?.page_fault ? ' [FAULT]' : ' [HIT]'}
                       </span>
                     )}
                     {access.type === 'process_created' && (
                       <span>Created Process {access.pid} ({access.pages} pages)</span>
                     )}
                     {access.type === 'process_terminated' && (
                       <span>Terminated Process {access.pid}</span>
                     )}
                     {access.type === 'algorithm_changed' && (
                       <span>Algorithm changed to {access.algorithm}</span>
                     )}
                     {access.type === 'demo_run' && (
                       <span>Demo executed ({access.results} accesses)</span>
                     )}
                     {access.type === 'random_access' && (
                       <span>Random access ({access.count} requests, {access.results} results)</span>
                     )}
                   </div>
                 </div>
               ))}
             ...
             <div className="access-heatmap">
               <h4>Page Access Frequency</h4>
               <div className="heatmap-grid">
                 {Array.from({ length: 16 }, (_, i) => {
                   const pageAccesses = accessHistory.filter(access => 
                     access.type === 'address_translation' && 
                     Math.floor(access.virtualAddress / 4096) === i
                   ).length;
                   const intensity = Math.min(pageAccesses / 5, 1);
                   return (
                     <div 
                       key={i}
     ...
     {showPerformanceChart && (
       <div className="comparison-modal">
         <div className="modal-content">
           <div className="modal-header">
             <h3><TrendingUp size={20} /> Performance Chart</h3>
             <button onClick={() => setShowPerformanceChart(false)} className="close-btn">×</button>
           </div>
           <div className="comparison-table">
             <div className="chart-section">
               <h4>Page Faults Over Time</h4>
               <div className="chart-container">
                 <svg width="100%" height="200" viewBox="0 0 400 200">
                   {performanceChart.pageFaults.map((faults, index) => {
                     const x = (index / Math.max(performanceChart.pageFaults.length - 1, 1)) * 380 + 10;
                     const maxFaults = Math.max(...performanceChart.pageFaults, 1);
                     const y = 190 - (faults / maxFaults) * 180;
                     return (
                       <g key={index}>
                         <circle cx={x} cy={y} r="3" fill="#e11d48" />
                         {index > 0 && (
                           <line
                             x1={(index - 1) / Math.max(performanceChart.pageFaults.length - 1, 1) * 380 + 10}
                             y1={190 - (performanceChart.pageFaults[index - 1] / maxFaults) * 180}
                             x2={x}
                             y2={y}
                             stroke="#e11d48"
                             strokeWidth="2"
             ...
             <div className="chart-section">
               <h4>Hit Ratio Over Time</h4>
               <div className="chart-container">
                 <svg width="100%" height="200" viewBox="0 0 400 200">
                   {performanceChart.hitRatios.map((ratio, index) => {
                     const x = (index / Math.max(performanceChart.hitRatios.length - 1, 1)) * 380 + 10;
                     const y = 190 - (ratio / 100) * 180;
                     return (
                       <g key={index}>
                         <circle cx={x} cy={y} r="3" fill="#059669" />
                         {index > 0 && (
                           <line
                             x1={(index - 1) / Math.max(performanceChart.hitRatios.length - 1, 1) * 380 + 10}
                             y1={190 - (performanceChart.hitRatios[index - 1] / 100) * 180}
                             x2={x}
                             y2={y}
                             stroke="#059669"
                             strokeWidth="2"
     ...
     {showProcessLayouts && (
       <div className="comparison-modal">
         <div className="modal-content large-modal">
           <div className="modal-header">
             <h3><Database size={20} /> Process Memory Layouts</h3>
             <button onClick={() => setShowProcessLayouts(false)} className="close-btn">×</button>
           </div>
           <div className="comparison-table">
             {Object.entries(processLayouts).map(([pid, layout]) => (
               <div key={pid} className="process-layout">
                 <h4>Process {pid} Virtual Memory Layout</h4>
                 <div className="layout-stats">
                   <span>Total Pages: {layout.totalPages}</span>
                   <span>Allocated: {layout.allocatedPages}</span>
                   <span>Working Set: {layout.workingSetSize}</span>
                 </div>
                 <div className="virtual-memory-layout">
                   {layout.virtualPages.map((page) => (
                     <div 
                       key={page.pageNumber}
                       className={`virtual-page ${page.allocated ? 'allocated' : 'unallocated'} ${
                         page.valid ? 'valid' : 'invalid'
                       } ${page.dirty ? 'dirty' : ''} ${page.referenced ? 'referenced' : ''}`}
                       title={`Page ${page.pageNumber}: ${page.valid ? `Frame ${page.frame}` : 'Not in memory'}`}
                     >
                       <div className="page-number">P{page.pageNumber}</div>
                       {page.valid && <div className="frame-number">F{page.frame}</div>}
                       <div className="page-flags">
                         {page.dirty && <span className="flag dirty">D</span>}
                         {page.referenced && <span className="flag referenced">R</span>}
                       </div>
                     </div>
                   ))}
                 </div>
                 <div className="layout-legend">
                   <div className="legend-item allocated">Allocated</div>
                   <div className="legend-item valid">In Memory</div>
                   <div className="legend-item dirty">Dirty</div>
                   <div className="legend-item referenced">Referenced</div>
                 </div>
                 </div>
             ))}
           </div>
